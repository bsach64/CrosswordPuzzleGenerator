...
class Crossword:
    ...
    def create_order(self):
        ...
        for word_one in all_words:
            for word_two in all_words:
                if word_one != word_two:
                    if (
                        self.info[word_one].row == self.info[word_two].row
                        and self.info[word_one].column == self.info[word_two].column
                    ):
        ...
        for word in self.info:
            if len(self.board[self.info[word].row][self.info[word].column]) < 2:
                self.board[self.info[word].row][self.info[word].column] = (
                    str(self.info[word].order)
                    + self.board[self.info[word].row][self.info[word].column]
                )
    ...
    def reduce(self):
        ...
        transposed_board = [
            col for col in transposed_board if any(cell != " " for cell in col)
        ]
...
def can_place(current_word, board, row, column):
    ...
    for i in range(1, (len(first_half) + 2)):
        for j in range(-1, 2):
            if (
                row - i < 0
                or column + j < 0
                or column + j >= len(board[row - i])
                or board[row - i][column + j] != " "
            ):
                flag_vertical = False
                break
    ...
    if flag_vertical:
        return Placement(
            row=(row - len(first_half)), column=column, direction="v"
        ), True
    ...
    if flag_horizontal:
        return Placement(
            row=row, column=(column - len(first_half)), direction="h"
        ), True
    return None, False
